...
static func _enumerate_units(v: Vector3i) -> Array:
	var result = []
	if v.x: result.append(Vector3i(v.x, 0, 0))
	if v.y: result.append(Vector3i(0, v.y, 0))
	if v.z: result.append(Vector3i(0, 0, v.z))
	return result

const FaceMask = {
	Vector3i(1,0,0): 0xAA, Vector3i(0,1,0):0xCC, Vector3i(0,0,1):0xF0,
	Vector3i(-1,0,0): 0x55, Vector3i(0,-1,0):0x33, Vector3i(0,0,-1):0x0F}
const FaceShift = {
	Vector3i(1,0,0): 1, Vector3i(0,1,0):2, Vector3i(0,0,1):4,
	Vector3i(-1,0,0): -1, Vector3i(0,-1,0):-2, Vector3i(0,0,-1):-4}
				...
				[var t, var u]: # Edge
					if voxel_node.get_cell(map_position + snapping):
						if voxel_node.get_cell(map_position + t):
							...
							print({t=t,u=u,fmu=FaceMask[-u], fmt=FaceMask[t],new_mesh_id=new_mesh_id})
							do_paint_cell_action(voxel_node, new_box_position, new_mesh_id)
							return EditorPlugin.AFTER_GUI_INPUT_STOP
						elif voxel_node.get_cell(map_position + u):
							var new_box_position = map_position + t
							var new_mesh_id = FaceMask[t] | FaceMask[-u]
							print({t=t,u=u,fmt=FaceMask[-t], fmu=FaceMask[u],new_mesh_id=new_mesh_id})
							do_paint_cell_action(voxel_node, new_box_position, new_mesh_id)
							return EditorPlugin.AFTER_GUI_INPUT_STOP
				[var a, var b, var c]:
						print("Point")
						...
						if inormal.length_squared() == 1:
							...
							var v = uv[1]
							print({u=u,v=v})
							if (voxel_node.get_cell(edited_coord + u) and
								voxel_node.get_cell(map_position + v)):
								var new_mesh_id = mesh_id_bit(snapping) | mesh_id_bit(snapping - inormal*2) | mesh_id_bit(snapping - inormal*2 - u *2) | mesh_id_bit(snapping - inormal*2 - v *2)
